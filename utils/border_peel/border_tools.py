...
def border_peel_single(data, border_func, threshold_func, precentile=0.1, verbose=False):
    ...
    if len(data) == 0:
        if verbose:
            print("Empty dataset detected in border_peel_single")
...
def border_peel(data, border_func, threshold_func, max_iterations = 150, min_iterations = 3, mean_border_eps = -1,
    ...
    # 确保n_neighbors至少为1
    nbrs = NearestNeighbors(n_neighbors=max(1, len(current_data)-1)).fit(current_data)
    ...
    for t in range(max_iterations):
        ...
        # 检查border_values是否为None
        if border_values is None:
            if verbose:
                print("border_values is None, breaking")
            break
...
def merge_core_points(core_points, link_thresholds, original_indices, cluster_sets, verbose=False):
    ...
    try:
        # 确保n_neighbors至少为1
        nbrs = NearestNeighbors(n_neighbors=max(1, len(core_points) - 1)).fit(core_points, core_points)
        distances, indices = nbrs.kneighbors()
