...
Description  : 使用归一化互信息(NMI)评估多种聚类算法的性能
'''
import os
import sys
...
def plot(filename, X, cluster_labels, predicted_labels):
    # 计算 NMI
    nmi = normalized_mutual_info_score(cluster_labels, predicted_labels)

    # 创建一个网格规范 (GridSpec)，用于更精确地控制布局
    gs = gridspec.GridSpec(1, 3, width_ratios=[1, 1, 0.2])  # 第三个列是窄的，用来放置NMI文本
...
if __name__ == '__main__':
    ...
    if update_performance == True:
        ...
        select_performance_folder = nmi_file  # f2.2. 选择使用的指标
        compute_performance = nmi_fun
    ...
    for filename in os.listdir(folder):
        ...
        if update_performance == True:
            ...
            if select_performance_folder == nmi_file:
